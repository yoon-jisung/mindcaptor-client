...
export default function App() {
  ...
  const getAccessToken = async(authorizationCode)=> {
    let resp = await axios.post('http://localhost:4000/googlelogin', { authorizationCode: authorizationCode })
    setAccessToken({ accessToken: resp.data.accessToken });
  }
  return (
    <div>
      <Switch>
        <Route path="/" exact={true} render={() => <Main loginHandler = {loginHandler} />} />
